#' Return testbox directional data from Verifit xml to data frame
#'
#' Takes an input argument of an xml file as exported by the Verifit, and
#' returns a data frame containing testbox directional results. Data frame is in
#' long format.
#'
#' @param filename An xml file exported from the Verifit
#' @return A data frame containing the testbox directional results.
#' @examples
#' \dontrun{
...
#' }
#'
#' @export boxdirectional_to_df

boxdirectional_to_df <- function(filename){
  testtype <- 'directional'
  unit <- 'dBspl'
  scale <- '12th'
